package com.rslakra.interview.finfare;
...
public class Finfare {
    ...
    public void preProcessor(List<String> transactionLogs) {
        for (String trx : transactionLogs) {
            ...
            List<Transaction> accountTrxs = transactions.getOrDefault(tx.account, new ArrayList<>());
            accountTrxs.add(tx);
            transactions.put(tx.account, accountTrxs);
